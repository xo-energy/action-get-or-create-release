...
function md5(bytes2) {
  ...
  return crypto2.createHash("md5").update(bytes2).digest();
}
var md5_default;
var init_md5 = __esm({
  "node_modules/uuid/dist/esm-node/md5.js"() {
...
function sha1(bytes2) {
  ...
  return crypto3.createHash("sha1").update(bytes2).digest();
}
var sha1_default;
var init_sha1 = __esm({
  "node_modules/uuid/dist/esm-node/sha1.js"() {
...
var require_dist_node2 = __commonJS({
  "node_modules/@octokit/endpoint/dist-node/index.js"(exports) {
    ...
    function expand(template2, context3) {
      var operators = ["+", "#", ".", "/", ";", "?", "&"];
...
var require_lodash2 = __commonJS({
  "node_modules/lodash.capitalize/index.js"(exports, module) {
    var INFINITY = 1 / 0;
    var symbolTag = "[object Symbol]";
    var rsAstralRange = "\\ud800-\\udfff";
...
var require_hosts_config = __commonJS({
  "node_modules/issue-parser/lib/hosts-config.js"(exports, module) {
    module.exports = {
      github: {
        actions: {
...
var require_issue_parser = __commonJS({
  "node_modules/issue-parser/index.js"(exports, module) {
    var escapeRegExp2 = require_lodash();
    var capitalize = require_lodash2();
    var isString = require_lodash3();
    var isPlainObject = require_lodash4();
    var uniqBy = require_lodash5();
    var hostConfig = require_hosts_config();
    var { hasOwnProperty } = Object.prototype;
    var FENCE_BLOCK_REGEXP = /^(([ \t]*`{3,4})([^\n]*)([\s\S]+?)(^[ \t]*\2))/gm;
    var CODE_BLOCK_REGEXP = /(`(?![\\]))((?:.(?!\1(?![\\])))*.?)\1/g;
    var HTML_CODE_BLOCK_REGEXP = /(<code)+?((?!(<code|<\/code>)+?)[\S\s])*(<\/code>)+?/gim;
    var LEADING_TRAILING_SLASH_REGEXP = /^\/?([^/]+(?:\/[^/]+)*)\/?$/;
    var TRAILING_SLASH_REGEXP = /\/?$/;
    function inverse(str) {
      return str.split("").reverse().join("");
    }
    function join(keywords) {
      return keywords.filter(Boolean).map(escapeRegExp2).join("|");
    }
    ...
    function buildRefRegexp({ actions, delimiters, issuePrefixes, issueURLSegments, hosts }) {
      ...
      )}))?(?:[^\\w\\n\\v\\r]|^|(?: |\\t)*(?:${join([" ", "	", ...delimiters])})(?: |\\t)*)${hosts.length > 0 ? `(?:${join(hosts)})?` : ""}((?:(?:[\\w-\\.]+)\\/)+(?:[\\w-\\.]+))?(${join([...issuePrefixes, ...issueURLSegments])})(\\d+)(?!\\w)`;
    }
    function buildRegexp(opts) {
      return new RegExp(
        opts.mentionsPrefixes.length > 0 ? `(?:${buildRefRegexp(opts)}|${buildMentionsRegexp(opts)})` : buildMentionsRegexp(opts),
        "gim"
      );
    }
    ...
    function parse4(text, regexp, mentionRegexp, { actions, issuePrefixes, hosts }) {
      let parsed;
      const results = {
      ...
      while ((parsed = regexp.exec(noCodeBlock)) !== null) {
        ...
        prefix = prefix && issuePrefixes.some((issuePrefix) => issuePrefix.toUpperCase() === prefix.toUpperCase()) ? prefix : void 0;
        raw = parsed[0].slice(
          parsed[0].indexOf(
    ...
    function normalize(opts, parentOpt) {
      for (const opt of Object.keys(opts)) {
        if (!parentOpt && opt === "actions") {
          ...
          if (opts[opt].length !== 0 && !opts[opt].every((opt2) => isString(opt2))) {
            throw typeError(parentOpt, opt);
          }
    ...
    module.exports = (options = "default", overrides = {}) => {
      ...
      return (text) => {
        ...
        const results = parse4(text, regexp, mentionRegexp, opts);
...
Writer.prototype.render = function render(template2, view, partials, config) {
  ...
  return this.renderTokens(tokens, context3, partials, template2, config);
};
Writer.prototype.renderTokens = function renderTokens(tokens, context3, partials, originalTemplate, config) {
  var buffer = "";
  ...
  for (var i = 0, numTokens = tokens.length; i < numTokens; ++i) {
    ...
    token = tokens[i];
    symbol = token[0];
    if (symbol === "#") value = this.renderSection(token, context3, partials, originalTemplate, config);
    else if (symbol === "^") value = this.renderInverted(token, context3, partials, originalTemplate, config);
    else if (symbol === ">") value = this.renderPartial(token, context3, partials, config);
    else if (symbol === "&") value = this.unescapedValue(token, context3);
    else if (symbol === "name") value = this.escapedValue(token, context3, config);
...
Writer.prototype.renderSection = function renderSection(token, context3, partials, originalTemplate, config) {
  var self2 = this;
  var buffer = "";
  var value = context3.lookup(token[1]);
  function subRender(template2) {
    return self2.render(template2, context3, partials, config);
  }
  if (!value) return;
  if (isArray(value)) {
    for (var j = 0, valueLength = value.length; j < valueLength; ++j) {
      buffer += this.renderTokens(token[4], context3.push(value[j]), partials, originalTemplate, config);
    }
  } else if (typeof value === "object" || typeof value === "string" || typeof value === "number") {
    buffer += this.renderTokens(token[4], context3.push(value), partials, originalTemplate, config);
    ...
    buffer += this.renderTokens(token[4], context3, partials, originalTemplate, config);
  }
  return buffer;
};
Writer.prototype.renderInverted = function renderInverted(token, context3, partials, originalTemplate, config) {
  var value = context3.lookup(token[1]);
  if (!value || isArray(value) && value.length === 0)
    return this.renderTokens(token[4], context3, partials, originalTemplate, config);
};
...
Writer.prototype.renderPartial = function renderPartial(token, context3, partials, config) {
  if (!partials) return;
  var tags = this.getConfigTags(config);
  ...
  if (value != null) {
    ...
    return this.renderTokens(tokens, context3, partials, indentedValue, config);
  }
};
...
Writer.prototype.escapedValue = function escapedValue(token, context3, config) {
...
// src/notes.js
var issueParser = (0, import_issue_parser.default)("github");
var template = `
# Changes
{{body}}
{{#commits}}
...
async function getCommits(github, base, head) {
  ...
  try {
    ...
    core2.warning(`Failed to compare commits: ${e.message}`);
  }
  core2.info(`Collected ${commits.length} since ${base}`);
  return commits;
}
async function getReleaseNotes(github, body, base, head) {
  ...
  const commits = commitsRaw.map((x) => {
    ...
    return {
      ...
      author: `[${x.commit.author.name}](https://github.com/${import_github.context.repo.owner}/${import_github.context.repo.repo}/commits?author=${x.author.login})`,
...
async function getOrCreateRelease(github) {
  try {
    const response = await github.rest.repos.getReleaseByTag({
      ...
      tag: inputs_default.tagName
    });
    core3.info(`Release ${inputs_default.tagName} already exists.`);
    return response.data;
  } catch (e) {
    if (e.status !== 404) {
      throw e;
    }
    let body = inputs_default.body || "TBA";
    if (inputs_default.previousReleaseSha) {
      core3.info("Generating release notes...");
      body = await getReleaseNotes(
        github,
        inputs_default.body,
        inputs_default.previousReleaseSha,
        inputs_default.targetCommitish
      );
    }
    core3.info(`Creating release ${inputs_default.tagName}`);
    const response = await github.rest.repos.createRelease({
      owner: import_github2.context.repo.owner,
      repo: import_github2.context.repo.repo,
      tag_name: inputs_default.tagName,
      target_commitish: inputs_default.targetCommitish,
      name: inputs_default.releaseName || inputs_default.tagName,
      body,
      prerelease: inputs_default.prerelease,
      draft: true
    });
    return response.data;
  }
...
async function uploadAssets(github, uploadUrl, patterns) {
  ...
  for (let i = 0; i < files.length; i += 1) {
    const file = files[i];
    const contentType = import_mime_types.default.lookup(file);
    const stat = fs.statSync(file);
    const read = fs.createReadStream(file);
    core3.info(`Uploading asset '${file}' (${(0, import_bytes.default)(stat.size)} ${contentType})`);
...
async function run() {
  try {
    const github = (0, import_github2.getOctokit)(inputs_default.githubToken);
    const release = await getOrCreateRelease(github);
    if (inputs_default.assets) {
      await uploadAssets(github, release.upload_url, inputs_default.assets);
    }
    if (release.draft && inputs_default.publish) {
      core3.info(`Publishing release ${release.name}`);
    ...
    core3.setOutput("release_id", release.id.toString());
    core3.setOutput("upload_url", release.upload_url);
  } catch (e) {
